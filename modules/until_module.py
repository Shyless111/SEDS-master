...
class MILNCELoss_BoF(nn.Module):
    ...
    def forward(self, sim_matrix, batch_size, n_video, n_text):
        if sim_matrix.size(0) // batch_size == n_video:
            kron_shape = np.ones((n_video, n_text))
        else:
            kron_shape = np.ones((n_text, n_video))

        mm_mask = np.eye(batch_size)
        mm_mask = np.kron(mm_mask, kron_shape)
        mm_mask = torch.tensor(mm_mask).float().bool().to(sim_matrix.device)
...
class KLdivergence(nn.Module):
    ...
    def forward(self, sampled_video_features, video_logsigma, sampled_text_features, text_logsigma):
        return self.kl_divergence(sampled_video_features.mean(dim=1), video_logsigma) + \
