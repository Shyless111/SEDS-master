...
def tensor_video_to_text_sim(sim_tensor):
    """Collapse a 3D similarity tensor into a 2D video-to-text similarity matrix.

    For multi-sentence-per-video evaluation we keep, for each video/text pair,
    the best score across the sentence dimension with a hard max.
    """
    if not torch.is_tensor(sim_tensor):
      sim_tensor = torch.tensor(sim_tensor)
    # Code to avoid nans
...
def tensor_video_to_text_metrics(sim_tensor):
    """Compute video-to-text retrieval metrics from a 3D similarity tensor."""
    return compute_metrics(tensor_video_to_text_sim(sim_tensor))
