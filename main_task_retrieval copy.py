...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss, loss_tv, loss_tp, loss_vp = model(input_ids, segment_ids, input_mask, right_batch, left_batch, body_batch)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info("Epoch:%d/%s, Step:%d/%d, Loss:%f, Loss_tv:%f, Loss_tp:%f, Loss_vp:%f, T/S:%.3f", epoch + 1,
                            args.epochs, step + 1,
...
def _run_on_single_gpu_new_mix(model, batch_list_v, batch_list_t, batch_sequence_output_list, batch_visual_output_pose_list, batch_visual_output_rgb_list, is_train, hybird=False, alpha=0.5, dual_mix=0.5):
    ...
    with torch.no_grad():
        ...
        for idx1, b1 in enumerate(batch_list_v):
            pose_global = batch_visual_output_pose_list[idx1]
            video_global = batch_visual_output_rgb_list[idx1]
            each_row_tv = []
            each_row_tp = []
            for idx2, b2 in enumerate(batch_list_t):
                text_global = batch_sequence_output_list[idx2]

                sim_tv, sim_tp = model.get_global_similarity(text_global, pose_global, video_global)
        ...
        return sim_matrix_tv, sim_matrix_tp
...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            if multi_sentence_:
                ...
                mask, pose_global, video_global = model.get_visual_output(right_batch, left_batch, body_batch, shaped=True, get_hidden=True)
                batch_visual_output_pose_list.append(pose_global)
                batch_visual_output_rgb_list.append(video_global)
                ...
                if len(filter_inds) > 0:
                    ...
                    text_global = model.get_sequence_output(input_ids, segment_ids, input_mask, get_hidden=True)
                    batch_sequence_output_list.append(text_global)
    ...
    #########################################
        ...
        sim_matrix_tv, sim_matrix_tp = _run_on_single_gpu_new_mix(model, batch_list_v, batch_list_t, batch_sequence_output_list, batch_visual_output_pose_list,\
