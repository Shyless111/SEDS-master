...
class ph_DataLoader_pose(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_pose:
            sentance_id = self.video_RGB_dict[idx][0]
            sample_text = self._get_text(sentance_id)
            return {'text': sample_text, 'RGB': video_feature, 'video_mask': video_mask}
...
def ph_pose_collate_fn(batch, padding=6):
    ...
    if 'right' not in batch[0]:
        for i in range(len(batch)):
            batch_RGB.append(batch[i]['RGB'])
            batch_body.append(batch[i]['video_mask'])
        ...
        return {'RGB_feature': torch.stack(batch_RGB, dim=0).float(),
                'body_mask': torch.stack(batch_body, dim=0).long(),
