...
from modules.tokenization_adapters import HFTokenizerAdapter
...
def get_args(description='CLCL on Retrieval Task'):
    ...
    parser.add_argument("--filip_loss_weight", default=0.5, type=float, help="Weight for FILIP fine-grained alignment loss.")
    ...
    parser.add_argument("--use_pose", action='store_true', help="Use pose encoder and pose losses. Default is video-text only.")
    parser.add_argument("--freeze_exfusion", action='store_true', help="Only optimize fusion loss when set.")
...
def move_batch_to_device(batch, device, use_pose):
    if use_pose:
        return {k: v.to(device) for k, v in batch.items()}

    keep_keys = {"RGB_feature", "body_mask", "pairs_text", "pairs_mask", "pairs_segment"}
    return {k: v.to(device) for k, v in batch.items() if k in keep_keys}
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    weight_decay = 0.001 if args.use_uatvr_head else 0.001
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        if n_gpu == 1:
            ...
            batch = move_batch_to_device(batch, device, args.use_pose)
        ...
        if args.use_pose:
            ...
            right_batch, left_batch = None, None
            body_batch = {'mask':sample['body_mask'], 'rgb':sample['RGB_feature']}
        ...
        for idx2, b2 in enumerate(batch_list_t):
            ...
            for idx1, b1 in enumerate(batch_list_v):
                ...
                sim_tv, sim_tp, sim_tf = model.get_global_similarity(
                    ...
                    video_hidden=video_outputs.get("hidden"),
                    video_valid_mask=video_outputs.get("valid_mask"),
                )

                each_row_tv.append(sim_tv.cpu().detach().numpy())
                if sim_tp is not None:
                    each_row_tp.append(sim_tp.cpu().detach().numpy())
                if sim_tf is not None:
                    each_row_tf.append(sim_tf.cpu().detach().numpy())

            sim_matrix_tv.append(np.concatenate(tuple(each_row_tv), axis=-1))
            if each_row_tp:
                sim_matrix_tp.append(np.concatenate(tuple(each_row_tp), axis=-1))
            if each_row_tf:
                sim_matrix_tf.append(np.concatenate(tuple(each_row_tf), axis=-1))

        return sim_matrix_tv, sim_matrix_tp or None, sim_matrix_tf or None
...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            if multi_sentence_:
                ...
                visual_outputs = model.get_visual_output(right_batch, left_batch, body_batch, shaped=True, get_hidden=True)
                mask = visual_outputs["mask"]
                pose_global = visual_outputs["pose_global"]
                if args.use_pose:
                    batch_visual_output_pose_list.append(pose_global)
                batch_visual_output_rgb_list.append({
                    "global": visual_outputs["video_global"],
                    "hidden": visual_outputs.get("video_hidden"),
                    "valid_mask": visual_outputs.get("video_valid_mask"),
                })
                ...
                if len(filter_inds) > 0:
                    input_ids, segment_ids, input_mask = input_ids[filter_inds, ...], segment_ids[filter_inds, ...], input_mask[filter_inds, ...]
                    text_outputs = model.get_sequence_output(input_ids, segment_ids, input_mask, get_hidden=True)
                    batch_sequence_output_list.append(text_outputs)
                    batch_list_t.append(None)
                total_text_num += b
            else:
    ...
    #########################################
        ...
        pose_outputs = batch_visual_output_pose_list if args.use_pose else None
        sim_matrix_tv, sim_matrix_tp, sim_matrix_tf = _run_on_single_gpu_new_mix(model, batch_list_v, batch_list_t, batch_sequence_output_list, pose_outputs,\
                                                           batch_visual_output_rgb_list, is_train=False, dual_mix=args.dual_mix)

        sim_matrix_tv = np.concatenate(tuple(sim_matrix_tv), axis=0)
        if args.use_pose:
        ...
        if args.uatvr_use_dsl:
            ...
            sim_matrix_tv = apply_dual_softmax(sim_matrix_tv)
            if args.use_pose:
                sim_matrix_tp = apply_dual_softmax(sim_matrix_tp)
                sim_matrix_tf = apply_dual_softmax(sim_matrix_tf)

        if args.do_eval:
            logger.info(f"before reshape, sim matrix save to {args.output_dir}")
            pkl.dump(sim_matrix_tv, open(os.path.join(args.output_dir, "sim_matrix_tv_before.pkl"), 'wb'))
            if args.use_pose:
                pkl.dump(sim_matrix_tp, open(os.path.join(args.output_dir, "sim_matrix_tp_before.pkl"), 'wb'))
                pkl.dump(sim_matrix_tf, open(os.path.join(args.output_dir, "sim_matrix_tf_before.pkl"), 'wb'))

        logger.info("before reshape, sim matrix size: {} x {}".format(sim_matrix_tv.shape[0], sim_matrix_tv.shape[1]))
        ...
        if args.do_eval:
            ...
            pkl.dump(sim_matrix_tv, open(os.path.join(args.output_dir, "sim_matrix_tv_after.pkl"), 'wb'))
            if args.use_pose:
                pkl.dump(sim_matrix_tp, open(os.path.join(args.output_dir, "sim_matrix_tp_after.pkl"), 'wb'))
                pkl.dump(sim_matrix_tf, open(os.path.join(args.output_dir, "sim_matrix_tf_after.pkl"), 'wb'))

        logger.info("after reshape, sim matrix size: {} x {} x {}".
                    format(sim_matrix_tv.shape[0], sim_matrix_tv.shape[1], sim_matrix_tv.shape[2]))
        ...
        if args.use_pose:
            ...
            logger.info("Fusion_Video-to-Text:")
            logger.info('\t>>>  R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}'.
                        format(v2t_metrics_tf['R1'], v2t_metrics_tf['R5'], v2t_metrics_tf['R10'], v2t_metrics_tf['MR'], v2t_metrics_tf['MeanR']))
        ...
        logger.info("Video_Video-to-Text:")
        logger.info('\t>>>  R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}'.
                    format(v2t_metrics_tv['R1'], v2t_metrics_tv['R5'], v2t_metrics_tv['R10'], v2t_metrics_tv['MR'], v2t_metrics_tv['MeanR']))
...
def main():
    ...
    if args.text_encoder_path:
        tokenizer = HFTokenizerAdapter(args.text_encoder_path)
    else:
